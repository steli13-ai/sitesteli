...
const Select = React.forwardRef(({
    ...
    // Fallback: allow passing native <option> children when options prop is empty
    const childOptions = React.useMemo(() => {
        if (!children) return [];
        return React.Children.toArray(children)
            .filter(child => React.isValidElement(child) && child.type === 'option')
            .map(child => ({ value: child.props?.value ?? child.props?.children, label: child.props?.children }));
    }, [children]);

    const combinedOptions = options && options.length > 0 ? options : childOptions;
    ...
    const getSelectedDisplay = () => {
        ...
        if (multiple) {
            const selectedOptions = combinedOptions?.filter(opt => value?.includes(opt?.value));
